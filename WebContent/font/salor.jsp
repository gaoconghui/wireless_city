																	...
																	for (var i = 0; i < length; i++) {
																		...
																		if ($data[i].type != 5) {
																			html += "<p data-attrid='" + $data[i].id + "'>"
																					...
																					+ $data[i].name
																					+ ":</span><input type='text' placeholder='" + type + "'/>"
																					+ "</p>";
																		} else {
																			var args = [];
																			args = $data[i].value
																					...
																					success : function(
																						...
																						if (data == 0) {
																							...
																							/* var html = "<div class='l_content' style='display:none;'>" + 
																								...
																								"	<div class='l_state'> 该资源正在审核中</div>"+
																								"	<div class='l_time'>" + new Date().format("yyyy-MM-dd HH:mm:ss") + " (" + getTimeFormat(new Date()) + ")" + 
																								"	</div>" + 
																								"	<div class='l_operation'>" + 
																							...
																							var flag=true;
																							$("#reference").find("a").each(function(){
																								if($(this).text()==categoryName)flag=false;
																							});
																							if(flag){
																								$("#reference").append("<a href='SearchProviderResourceAction_searchResourceBycatalogue?catalogueId="+pid+"'>"+categoryName+"</a>");
																							} */
		...
		function dialogShow(obj) {
			...
			$dialog
					...
					.on(
							"click",
							function() {
								var deleteid = $(obj).closest(".l_content")
										.data("rid");
								$
										.ajax({
											data : {
