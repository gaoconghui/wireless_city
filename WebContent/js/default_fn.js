...
function searchInitialize() {
	...
	$("#hd_attrref").on({
		...
		"mouseleave" : function() {
			...
			$(this).find(".delete_icon").css("color", "#ccc");
		}
	}, ".attr_show");
	// 查询条数
...
function registerValidate() {
	...
	$("#userform").find("input[name=name]").blur(
			function() {
				...
				if (isEmpty(content)) {
					...
					$this.next().find(".icon").css("color", "#dc6666").html(
							"&#xe602;");
	...
	// loginPwd
	$("#c_table").find("input[name=loginPwd]").blur(
			function() {
				var $this = $(this);
				var content = $this.val();
				if (isEmpty(content)) {
					...
					$this.next().find(".info").text("");
					$this.next().find(".icon").css("color", "#87d448").html(
							"&#xe60c;");
					$this.attr("validate", "1");
				}
			});
	// confirmPwd
	$("#c_table").find("input[name=confirmPwd]").blur(
			function() {
				var $this = $(this);
				var content = $this.val();
				var pwd = $this.parents("form").find("input[name=loginPwd]")
						.val();
				if (isEmpty(content)) {
					$this.next().find(".info").text("不允许为空！");
					$this.next().find(".icon").css("color", "#dc6666").html(
	...
	$("#c_table")
			...
			.blur(
					function() {
						...
						if (isEmpty(content)) {
							$this.next().find(".info").text("不允许为空！");
							$this.next().find(".icon").css("color", "#dc6666")
									.html("&#xe602;");
							$this.attr("validate", "0");
						} else {
							$this.next().find(".info").text("");
							$this.next().find(".icon").css("color", "#87d448")
									.html("&#xe60c;");
							$this.attr("validate", "1");
						}
					});
	// industry_name(别名name);
	$("#providerform").find("input[name=name]").blur(
			function() {
				var $this = $(this);
				var content = $this.val();
				if (isEmpty(content)) {
	...
	// email
	$("#c_table").find("input[name=email]").blur(
			function() {
				var $this = $(this);
				var content = $this.val();
				if (isEmpty(content)) {
	...
	// QQ
	$("#c_table").find("input[name=qq]").blur(
			function() {
				var $this = $(this);
				var content = $this.val();
				if (isEmpty(content)) {
	...
	// post code
	$("#c_table").find("input[name=postcode]").blur(
			function() {
				var $this = $(this);
				var content = $this.val();
				if (isEmpty(content)) {
