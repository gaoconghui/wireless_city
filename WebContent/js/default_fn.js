																		...
																		.ajax({
																			...
																			success : function(
																				...
																				if (data == 0) {
																					...
																					location.href = "SearchProviderResourceAction_searchResourceBycatalogue?catalogueId="
																							+ pid
																							+ "";
																				}
																			}
	...
	function dialogShow(obj) {
		...
		$dialog
				...
				.on(
						"click",
						function() {
							var deleteid = $(obj).closest(".l_content").data(
									"rid");
							$
									.ajax({
										data : {
					...
					function() {
						...
						$("#cancle")
								.click(
										function() {
											var id = $("#rs_id").text();
											location.href = "AddResourceAction_toUpdateResource?id="
													+ id;
										});
						// 审核和时间消失
						$("#updatetime,#state").remove();
						// span.value变为input
						var name_content = $("#update_rs").find(".name").find(
								".value").text();
						$("#update_rs").find(".name").html(
								"<span class='key'>资源名称</span><input name='resource_name' value='"
										+ name_content + "'/>");
						var desc_content = $("#update_rs").find(".desc").find(
								".value").text();
						$("#update_rs").find(".desc").html(
								"<span class='key'>资源描述</span><textarea name='description'>"
										+ desc_content + "</textarea>");
						$("#update_rs")
								.find(".right_wrapper")
								.find("p")
								.each(
										function() {
											var type = $(this).find(".value")
													.data("type");
											var content = $(this)
													.find(".value").attr(
															"title");
											var name = $(this).find(".key")
													.attr("title");
											var name_index = $(this).find(
													".value").data("name");
											var args = [];
											if (type == "0") {
												$(this)
												...
												$(this)
														.html(
																"<span class='key' title='"
																		...
																		+ "'>"
																		+ name
																		+ "</span><select name='resourceAttrs["
																		+ name_index
																		+ "].value'>"
																		+ selHtml
																		+ "</select>");
...
$("#hd_attribute").find(".inner").each(
		function() {
			...
			$(this).find(".sub_btn").each(
					function() {
						$(this).off("click").click(
								function() {
									var min = $prices.find("input[name=min]")
											.val();
									var max = $prices.find("input[name=max]")
											.val();
									if (!numCheck(min) || !numCheck(max)) {
										var errortxt = $info.data("error");
										$info.find("i").css("color",
												"rgb(220, 102, 102)").html(
												"&#xe602;");
										$info.find("span").text(errortxt);
										return false;
									}
									if (min * 1 >= max * 1) {
										$(this).parents(".bd_prices").find(
												".info").find("i").css("color",
												"rgb(220, 102, 102)").html(
												"&#xe602;");
										$(this).parents(".bd_prices").find(
...
function navOperation() {
	...
	$("#nav_ul").find("li").each(function() {
		if (isEmpty($(this).data("sel"))) {
			$(this).on({
				"mouseenter" : function() {
					$(this).addClass("lidemohover");
				},
				"mouseleave" : function() {
					$(this).removeClass("lidemohover");
				}
			});
		}
	});
	...
	/* left_nav hover */
	/*
	 * $("#left_nav").on({ "mouseenter" : function() { var _index =
	 * $(this).data("index"); $(this).addClass("lihover");
	 * $(this).find("a").css({ "color" : "#b61d1d" });
	 * $("#left_content").find("li").eq(_index).css("top",40 * _index +
	 * "px").show(); }, "mouseleave" : function(index) { var _index =
	 * $(this).data("index"); $(this).removeClass("lihover");
	 * $(this).find("a").css({"color" : "#fff"});
	 * $("#left_content").find("li").eq(_index).hide(); } }, "li");
	 */
	$("#left_nav").find("li").each(
			function(index) {
				if (index != 0) {
					$(this).on(
							{
								"mouseenter" : function() {
									...
									$("#left_content").find("li").eq(_index)
											.css("top", 40 * _index + "px")
											.show();
...
function registerValidate() {
	...
	// input blur
	// name
	$("#userform").find("input[name=name]").blur(
			function() {
				var $this = $(this), $next = $this.next();
				if ($this.parents(".placeholder").data("exist") == "1") {
					$next = $this.parents(".placeholder").next();
				...
				if (isEmpty(content)) {
					...
					$next.find(".icon").css("color", "#dc6666")
							.html("&#xe602;");
					...
					$.ajax({
						...
						success : function(data) {
							if (data == "0") {
								...
								$next.find(".info").text("");
								$next.find(".icon").css("color", "#87d448")
										.html("&#xe60c;");
	...
	// loginName
	$("#c_table").find("input[name=loginName]").blur(
			function() {
				var $this = $(this), $next = $this.next();
				if ($this.parents(".placeholder").data("exist") == "1") {
					$next = $this.parents(".placeholder").next();
	...
	$("#c_table").find("input[name=loginPwd]").blur(function() {
		...
		if (isEmpty(content)) {
			$next.find(".info").text("不允许为空！");
			$next.find(".icon").css("color", "#dc6666").html("&#xe602;");
			$this.attr("validate", "0");
			...
			$next.find(".info").text("");
			$next.find(".icon").css("color", "#87d448").html("&#xe60c;");
			$this.attr("validate", "1");
		}
	});
	// confirmPwd
	...
	// phoneNumber
	$("#c_table").find("input[name=phoneNumber],input[name=cellphone]").blur(
			function() {
				var $this = $(this), $next = $this.next();
				if ($this.parents(".placeholder").data("exist") == "1") {
					$next = $this.parents(".placeholder").next();
	...
	$("#c_table")
			.find(
					"input[name=realName],input[name=industryName],input[name=industry],input[name=scale],input[name=address]")
			.blur(
					function() {
						var $this = $(this), $next = $this.next();
						if ($this.parents(".placeholder").data("exist") == "1") {
	...
	// telephone
	$("#c_table").find("input[name=telephone],input[name=fax]").blur(
			function() {
				var $this = $(this), $next = $this.next();
				if ($this.parents(".placeholder").data("exist") == "1") {
					$next = $this.parents(".placeholder").next();
