...
function tmCheckImage(fileName) {
	return /(gif|jpg|jpeg|png|psd|GIF|JPG|PNG|PSD)$/ig.test(fileName);
}
/*验证是否为音乐*/
function tmCheckSong(fileName) {
	return /(MP3|WMA|WAV|MOD|mp3|wma|wav|mod)$/ig.test(fileName);
}

/*验证是否为种子*/
function tmCheckTorrent(fileName) {
	return /(torrent|TORRENT)$/ig.test(fileName);
}

/*验证是否为视频*/
function tmCheckVideo(fileName) {
	return /(avi|mp4|flv|wav)$/ig.test(fileName);
}
...
$.tmDate = {
 ...
 _transferDate : function(date){
	if(typeof date =="string"){
		return new Date(date.replace(/-/ig,"/").replace(/T/g," "));
	}else{
		return date;
	}
 },
